...
class URLStatsView(APIView):
    ...
    async def get(self, request, shortened_url):
        try:
            # Get original URL (this could be async if you change how URLShortenerService works)
            url = await URLShortenerService.get_original_url(shortened_url)
            if url:
                # Use sync_to_async to call the ORM method asynchronously for click_count
