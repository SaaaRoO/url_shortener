...
class TestURLShortener:
    ...
    async def setup(self, async_client):
        self.client = async_client  # Use the provided async_client fixture

    # Test for creating a shortened URL
    async def test_shorten_url(self):
        response = await self.client.post('/api/shorten/', {'original_url': 'https://example.com'})
    ...
    async def test_get_url_stats(self):
        ...
        response = await self.client.get(f'/api/stats/{shortened_url}/')
        assert response.status_code == status.HTTP_200_OK
        assert 'click_count' in response.json()

    # Test when the shortened URL does not exist
    async def test_get_non_existent_url(self):
    ...
    async def test_invalid_shorten_url(self):
        response = await self.client.post('/api/shorten/', {'original_url': ''})
